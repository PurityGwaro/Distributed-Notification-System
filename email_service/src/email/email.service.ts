...
export class EmailService implements OnModuleInit, OnModuleDestroy {
  ...
  private async connectRedis() {
    ...
    this.redisClient.on('connect', () => console.log('Email Service: Redis connected'));
  ...
  private async processEmailMessage(msg: any, channel: any) {
    ...
    try {
      ...
      if (attempts < 3) {
        ...
        console.log(`Retrying email (attempt ${attempts + 1}/3) after ${delay}ms`);
      } else {
    ...
    try {
      ...
      if (currentStatus) {
        ...
        const updatedStatus = {
          ...
          attempts: (statusObj.attempts || 0) + (status === NotificationStatus.RETRYING ? 1 : 0),
        };
      ...
      try {
        ...
        console.log(`Status updated via API Gateway: ${notificationId} -> ${status}`);
      } catch (apiError) {
        // Don't fail if API Gateway is unavailable - Redis update is sufficient
        console.warn('Failed to update status via API Gateway (non-critical):', apiError.message);
      }
